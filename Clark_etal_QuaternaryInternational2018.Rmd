...
subtitle: "LANDSCAPES, CLIMATE CHANGE & FORAGER MOBILITY IN THE UPPER PALEOLITHIC OF NORTHERN SPAIN"
...
This workflow is the output for an R Markdown script that performed all analyses used in the paper entitled **Landscapes, climate change & forager mobility in the Upper Paleolithic of northern Spain** (GA Clark corresponding author), submitted to _Quaternary International_, 2017. 
...
The complete dataset and the R Markdown script that produced this document can be downloaded at: _[URL for GitHub/Zenodo repository]_
  
This dataset should be cited as follows:  
  
_[Data citation goes here after obtaining DOI Zenodo]_
...
fit <- aov(formula = retouch.freq~period, data = subset(nwiberia.lithics, total.ret>0 & period != "Asturian"))

anovainfo <- paste("ANOVA: ", "F =", round(summary(fit)[[1]]$`F value`[[1]], digits=4), " p =", round(summary(fit)[[1]]$`Pr(>F)`[1], digits=5))

ggplot(subset(nwiberia.lithics, total.lithics>0 & period != "Asturian")) + 
...
ggplot() + 
  geom_point(data=subset(nwiberia.lithics, retouch.freq>0), aes(y=elev.modern, x=dist.coast.modern, size=retouch.freq, color=cut(retouch.freq, breaks=c(0,.1,.61), labels = c('expedient', 'curated'), include.lowest = T)), shape=1)  +
  geom_point(data=subset(nwiberia.lithics, is.na(retouch.freq)), aes(y=elev.modern, x=dist.coast.modern), size=1, color='grey50') +
  xlim(c(0,40)) +
...
(1200 x 600 for each image)  

```{r Aurignacian SPD calculation, warning=FALSE, message=FALSE, results="hide"}
aurignacian.spd <- with(nwiberia.dates[grep("Aurignacian", nwiberia.dates$attribution), ], BchronDensity(ages = C14.mean, ageSds = C14.sd, calCurves = calib.curve, numMix = 15, store_density=TRUE))

title <- paste("Aurignacian (N = ", nrow(nwiberia.dates[grep("Aurignacian", nwiberia.dates$attribution), ]), ")" )
...
```{r Gravettian SPD calculation, warning=FALSE, message=FALSE, results="hide"}
gravettian.spd <- with(nwiberia.dates[grep("Gravettian", nwiberia.dates$attribution), ], BchronDensity(ages = C14.mean, ageSds = C14.sd, calCurves = calib.curve, numMix = 15, store_density=TRUE))

title <- paste("Gravettian (N = ", nrow(nwiberia.dates[grep("Gravettian", nwiberia.dates$attribution), ]), ")" )
...
```{r Solutrean SPD calculation, warning=FALSE, message=FALSE, results="hide"}
solutrean.spd <- with(nwiberia.dates[grep("Solutrean", nwiberia.dates$attribution), ], BchronDensity(ages = C14.mean, ageSds = C14.sd, calCurves = calib.curve, numMix = 15, store_density=TRUE))

title <- paste("Solutrean (N = ", nrow(nwiberia.dates[grep("Solutrean", nwiberia.dates$attribution), ]), ")" )
...
```{r Magdalenian SPD calculation, warning=FALSE, message=FALSE, results="hide"}
magdalenian.spd <- with(nwiberia.dates[grep("Magdalenian", nwiberia.dates$attribution), ], BchronDensity(ages = C14.mean, ageSds = C14.sd, calCurves = calib.curve, numMix = 15, store_density=TRUE))

title <- paste("Magdalenian (N = ", nrow(nwiberia.dates[grep("Magdalenian", nwiberia.dates$attribution), ]), ")" )
...
```{r Azilian SPD calculation, warning=FALSE, message=FALSE, results="hide"}
azilian.spd <- with(nwiberia.dates[grep("Azilian", nwiberia.dates$attribution), ], BchronDensity(ages = C14.mean, ageSds = C14.sd, calCurves = calib.curve, numMix = 15, store_density=TRUE))

title <- paste("Azilian (N = ", nrow(nwiberia.dates[grep("Azilian", nwiberia.dates$attribution), ]), ")" )
...
```{r Mesolithic SPD calculation, warning=FALSE, message=FALSE, results="hide"}
mesolithic.spd <- with(nwiberia.dates[grep(paste(c("Asturian", "Meso"), collapse="|"), nwiberia.dates$attribution), ], BchronDensity(ages = C14.mean, ageSds = C14.sd, calCurves = calib.curve, numMix = 15, store_density=TRUE))

title <- paste("Mesolithic (N = ", nrow(nwiberia.dates[grep(paste(c("Asturian", "Meso"), collapse="|"), nwiberia.dates$attribution), ]), ")" )
...
```{r all industries SPD graph, warning=FALSE, message=FALSE, results="hide", fig.width=10, fig.height=6}
...
Eliminate all dates where CV for uncalibrated date >0.05 (90th percentile)
\
```{r create c14dates.agg, warning=FALSE, message=FALSE}
c14.aggdates <- with(na.omit(subset(nwiberia.dates, C14.cv<=0.05 & c("LUP art", "LUP art (Solutrean?)") %in% attribution)), aggregate(na.omit(subset(nwiberia.dates, C14.cv<=0.05 & c("LUP art", "LUP art (Solutrean?)") %in% attribution)), by = list(site.prov), FUN = mean))

nwiberia.dates.agg <- with(na.omit(subset(nwiberia.dates, C14.cv<=0.05 & c("LUP art", "LUP art (Solutrean?)") %in% attribution)), aggregate(na.omit(subset(nwiberia.dates, C14.cv<=0.05 & c("LUP art", "LUP art (Solutrean?)") %in% attribution)), by = list(site.prov), FUN = first))
...
```{r combined data SPD graph, warning=FALSE, message=FALSE, results="hide", fig.width=10, fig.height=6}
NWIb.ice.cores <- subset(ice.cores, years.BP<=45000 & years.BP>=5000)

ggplot() + 
  geom_line(aes(x=NWIb.dens$ageGrid, y=NWIb.dens$densities, color='age', lty='age'), lwd = 1) + 
